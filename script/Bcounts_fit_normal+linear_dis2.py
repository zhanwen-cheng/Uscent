...
def usage():
    print('''
          ...
          -d/--depth          input depth and B counts file
...
################################################################################
param_bounds=([-np.inf,-np.inf,max_y,(max(df_new.new_lable)+min(df_new.new_lable))/2 - 500,-np.inf],
    [np.inf,np.inf,max_y*max_y,(max(df_new.new_lable)+min(df_new.new_lable))/2 +500,np.inf])
print("fit begin")
